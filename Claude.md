...
**통일성이란**: 애슬레틱 카드의 기록·영상·경로 영상 템플릿에서 폰트 크기, 차트 크기, 여백, MiniMe 크기, 색상 등 **데이터 표시 요소의 속성이 동일**한 것. 배경·비율은 달라도 데이터는 같아 보여야 한다.

**구현 원칙**
- **단일 컴포넌트 원칙**: 영상/경로 영상 오버레이는 `VideoOverlayCard(scale:)` **하나만** 사용.
- **scale 파라미터**: 렌더 크기에 맞게 scale을 조정하되 `scale=1.0`의 기준값(폰트 pt, 차트 px, 여백 pt)은 애슬레틱 카드(기록 템플릿)와 동일하게 유지.
...
**C. 공유 카드**: 카드 선택(스탬프·원라이너·애슬레틱) + 템플릿 선택(기록·사진·영상·슬라이드·경로 영상) / 데이터 토글(레벨 기본+사용자) / 사진 배경 / 제목 편집 / ImageRenderer→ShareLink.
...
- [ ] 공유 카드 기록+사진 2종(데이터 토글·사진 배경) + ImageRenderer + ShareLink
...
- **타이포(공유 카드)**: 기록(콘덴스드) + 사진(세리프 이탤릭) 2종. 라벨=시스템 산세리프 대문자.
- **미니미**: 마스코트 베이스 + Image Playground 강화
