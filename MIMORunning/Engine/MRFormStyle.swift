...
///   말하는 조건은 두 단계다 — **관측 지표 2개 이상**(한 지표만으로는 "달리는 방식"을 논할 수 없다)
///   + 그중 **실증 변화 1개 이상**(`MRFormShift.isReal`). 실증이 0개면 안정으로 보고 침묵한다.
///   두 지표가 서로 같은 이야기를 할 필요는 없다 — 실증 1개면 그 지표만 놓고 말한다.
/// - Parameters:
