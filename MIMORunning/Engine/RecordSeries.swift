...
/// 성장 탭 "기록 카드"가 보여 주는 지표.
enum RecordMetric: CaseIterable {
    case distance   // km
    case time       // 분
    case load       // sRPE AU (강도 × 시간)
    case pace       // 기간 평균 대비 (위 = 빠름)
}
...
enum RecordSeries {
    ...
    /// 페이스 모드 막대 값 — baseline − pace (양수 = 평균보다 빠름). 페이스 없으면 nil.
    static func paceDelta(_ bar: RecordBar, baseline: Double) -> Double? {
        guard let p = bar.paceSec else { return nil }
        return baseline - p
    }
