...
| `0 < x < 1` | x ≥ 1은 감쇠가 아님 — 종료 직후 걷다가 멈춰 선 패턴(로그 정의역 보호) |
| `20 ≤ τ ≤ 300` | 남는 극단값. x로 환산하면 0.050 ≤ x ≤ 0.819 |

> τ 상한을 300초로 잡은 이유: 200초는 x ≤ 0.741에 해당해 "2분 뒤에도 계속 내려오는 중"인 러닝을 상당수 잘라낸다 — §2의 세 번째 구간이 바로 그 경우다. 문헌상 최대운동 후 τ는 대체로 30–120초이고 비훈련·더위에서 길어지므로, 300초는 "자율신경 감쇠로 보기 어려운 것"(쿨다운 조깅을 일정 강도로 이어간 경우 등)만 거른다.
...
8개 미만이면 **줄 자체를 그리지 않는다.** 원래는 "1분 −38 · 2분 −52bpm" 같은 원시 숫자 폴백을 냈으나, 이후 다음 이유로 없앴다.

- 게이팅이 어긋나 있었다: 폴백이 쓰는 두 숫자는 120초 샘플만 있으면 나오는데, 표시 여부는 τ가 가드(§1)를 통과했는지에 걸려 있었다 — 샘플 품질이 같은 두 러닝이 화면에 드러나는 이유를 알 수 없는 채로 갈릴 수 있었다.
- 같은 숫자를 상세 화면 `HRRecoveryPanelChart`(심박 패널의 회복 차트)가 이미 보여준다.
- §2-4(행동으로 이어지지 않는 데이터는 오락)와 앱의 전반적인 태도 — 자리표시자를 보여주느니 침묵한다.
...
- 과거 표본 7개 → `shapeCaption`이 nil(줄을 그리지 않음), 8개 → 비교 문구
- 캡션이 nil이면 `topCaptionH`도 1줄 높이로 돌아가는지 — 둘이 어긋나면 안 그리는 줄의 여백만 남는다
